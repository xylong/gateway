...
func (rule *HttpRule) Find(c *gin.Context, tx *gorm.DB) error {
	return tx.SetCtx(public.GetGinTraceContext(c)).Where(rule).Find(rule).Error
}

func (rule *HttpRule) Save(c *gin.Context, tx *gorm.DB) error {
...
func (rule *HttpRule) ListByServiceID(c *gin.Context, tx *gorm.DB, serviceID int64) ([]HttpRule, int64, error) {
	var list []HttpRule
	var count int64
